...
export async function POST(request: Request) {
  ...
  const total = Math.max(0, subtotal - bundleSavings - (discountAmount ?? 0)) + shippingAmount + tax;
  ...
  const combinedDiscount = Math.min(
    subtotal,
    (discountAmount ?? 0) + (bundleSavings > 0 ? bundleSavings : 0),
  );

  let stripeCouponId: string | undefined;
  if (combinedDiscount > 0) {
    const coupon = await stripe.coupons.create({
      amount_off: Math.round(combinedDiscount * 100),
      currency,
      name: discountCode ? `Discount ${discountCode}` : "Bundle discount",
      duration: "once",
    });
